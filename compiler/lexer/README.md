...
The lexer (lexical analyzer) is the first stage of the VibeLang compiler pipeline. It transforms source code into a stream of tokens.

## Overview

The lexer reads VibeLang source code character by character and groups them into meaningful tokens. It handles:
...
class Lexer:
        ...
        if self.peek() == '#':
            if self.peek(1) == '#':
                ...
                while not (self.peek() == '#' and self.peek(1) == '#'):
                    if self.peek() == '\0':
                        raise LexError("Unclosed multi-line comment")
                    self.advance()
    ...
    def read_string(self) -> Token:
        ...
        while self.peek() != '"':
            if self.peek() == '\0':
                raise LexError("Unclosed string literal")
        ...
        return Token(TokenType.STRING_LITERAL, value, self.line, start_column, 0)
...
VibeLang uses **2-space indentation**:
- Each indentation level is exactly 2 spaces
- Tabs are not allowed
- Mixed spaces and tabs raise an error
- Indentation must be consistent within blocks
...
Tokens generated:
```
DEFINE, IDENTIFIER(example), LPAREN, RPAREN, ARROW, UNIT, NEWLINE
GIVEN, NEWLINE
INDENT, WHEN, IDENTIFIER(condition), NEWLINE
INDENT, IDENTIFIER(doSomething), LPAREN, RPAREN, NEWLINE
DEDENT, OTHERWISE, NEWLINE
INDENT, IDENTIFIER(doSomethingElse), LPAREN, RPAREN, NEWLINE
DEDENT, DEDENT, EOF
```
...
- Invalid characters
- Incorrect indentation (not multiple of 2)
- Tabs in indentation
...
- Numeric literal suffixes (for type hints)
- Raw string literals
